              ...
              switch( telaAtual ){
                     ...
                     case TELA_PREPARACAO:
                          ...
                          if( mouse_b & 1 && ( mouse_x >= 0 && mouse_x <= 80 ) && ( mouse_y >= 517 && mouse_y <= 550 ) ){
                             ...
                             // Altera a imagem do bot�o iniciar quando passar o cursor do mouse em cima
                             ...
                             // Inicia o jogo ao clicar sobre o bot�o
                             if( mouse_b & 1 ){
...
void Inicializa() {
    ...
    // Inicia a comunica��o com a porta serial desejada.
...
void Finaliza(){
    ...
    // Finaliza todas as comunica��es com a porta serial.
...
int exibeJogo(){
  ...
  ***************************************************************************/
  ...
  /**************************************************************************
  Iniciando Bitmaps est�ticos do cen�rio
  ***************************************************************************/
  ...
  // Variaveis de controle do movimento da �gua
  ...
  int tipoMouse, posicaoGrade;
    ...
    while( !fimJogo ){
          ...
          // Insere o tabuleiro de cores para controle de posi��o no cen�rio
          desenhaCenario( buffer, stcGradeTabuleiroCores );          

          // Insere a �gua de fundo no cen�rio 
          desenhaCenario( buffer, stcFundoAgua );

          // Insere a �gua em movimento no cen�rio
          ...
          // Ap�s clicar no bot�o Menu, ser� exibido um alerta.
          if( pauseJogo ){

             desenhaCenario( buffer, stcMenuSair ); 
             tipoMouse = 0;

              // Op��o Sim - Volta para o menu inicial
...
int definePosicaoGrade( BITMAP *bmp ){
     ...
     // Recebe o valor do RGB da localiza��o do mouse na imagem definida.
...
void populaTabuleiroAdversario(){
  ...
  for(i = 0; i < qtdNavios; i ++){
    ...
    while(flagIndice){
      ...
      if(preparaTabuleiroAdversario(mtxTabuleiroAdversario[rLinha][rColuna].indice, direcao, tipoNavio[i]))
        flagIndice = 0;
    }
  }
...
//variaivel posi��o igual a 1 Vertical, igual a zero Horizontal. 
  ...
  for(linha = 0; linha < TAB_DIM; linha++){
    for(coluna = 0; coluna < TAB_DIM; coluna++ ){
      ...
      if(mtxTabuleiroAdversario.indice == indice){

        if(mtxTabuleiroAdversario.temNavio == 0){
...
void populaPreparacao
 ...
 Est� fun��o � responsavel por popular a matriz com as posi��es de X e Y
e os indices de cada uma das posi��es de acordo com a imagem do tabuleiro
da tela de prepara��o.
...
void populaTabuleiroJogo
 ...
 Est� fun��o � responsavel por popular a matriz com as posi��es de X e Y
e os indices de cada uma das posi��es de acordo com a imagem dos tabuleiros
principais do jogo.
